...
Pdr_Man_t* AbcMcInterface::setupPdrMan(Aig_Man_t* pMan)
{
    Pdr_Par_t pdrPars;
    Pdr_ManSetDefaultParams(&pdrPars);

    Pdr_Man_t *pPdr= Pdr_ManStart(pMan , &pdrPars, NULL);

    return pPdr;
}

void AbcMcInterface::ensurePdrFrames(Pdr_Man_t* pPdr, int nFrame)
{
    for (int i = Vec_PtrSize(pPdr->vSolvers); i <= nFrame; i++)
    {
        Pdr_ManCreateSolver(pPdr, i);
    }
}

void AbcMcInterface::addCubesToPdrFrame(Pdr_Man_t* pPdr, Vec_Ptr_t* pCubes, int nFrame)
{
    ensurePdrFrames(pPdr, nFrame);

    int j;
    Pdr_Set_t* pCube;
    Vec_PtrForEachEntry(Pdr_Set_t*, pCubes, pCube, j)
    {
        Vec_VecPush( pPdr->vClauses, nFrame, pCube );   // consume ref
        pPdr->nCubes++;

        // add clause
        for (int i = 1; i <= nFrame; i++ )
            Pdr_ManSolverAddClause( pPdr, i, pCube );
    }
}

void AbcMcInterface::getCubesFromPdrFrame(Pdr_Man_t* pPdr, int nFrame, Vec_Ptr_t* pCubes)
{
    int i;
    Vec_Ptr_t* vVec;
    Vec_VecForEachLevelStart(pPdr->vClauses, vVec, i, nFrame)
    {
        int j;
        Pdr_Set_t* pCube;
